...
impl RgbModel {
    ...
    fn add_tile(&self, tile_bytes: &Bytes, zoom_position_elevation: &Vec<u32>, zp_covered: &[Vec<u32>], bbox: &Bbox) -> Vec<(Vec<i32>, Vec<f64>, Vec<u32>)> {
        let decoder = png::Decoder::new(tile_bytes.as_ref());
        ...
        let sixteenth = self.get_sixteenth_tiles_coords(zoom_position_elevation);
        let zoom_position_covered_str_set: HashSet<String> = zp_covered.iter().map(|zp| {format!("{}/{}/{}", zp[0], zp[1], zp[2])}).collect();
        let sixteenth_pixel_range = get_sixteenth_pixel_ranges();
        ...
        for (index, zoom_pos_str) in sixteenth.iter().enumerate() {
            ...
            let px_range = &sixteenth_pixel_range[index];
            let mut elev = Vec::with_capacity(128 *128);
            ...
            let mut array = Vec::with_capacity(CONST_VERTICES * CONST_VERTICES * 3);
